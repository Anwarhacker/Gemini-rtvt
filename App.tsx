...
import { 
  Mic, MicOff, Settings, Activity, Volume2, Users, Eye, 
...
export default function App() {
  ...
  const [grammarModalData, setGrammarModalData] = useState<{ text: string, analysis: string } | null>(null);
  ...
  // PWA Install State
  const [deferredPrompt, setDeferredPrompt] = useState<any>({ prompt: () => {}, userChoice: Promise.resolve({ outcome: 'accepted' }) });
  ...
  const handleInstallApp = async () => {
    if (deferredPrompt) {
      ...
      alert('PWA installation is not supported in this environment. Deploy to HTTPS for full PWA functionality.');
    }
  };
  ...
  const exportTranscript = () => {
    const transcript = messages.map(msg => {
      const speaker = msg.speaker === 'System' ? (msg.isTranslation ? 'Translation' : 'AI') : msg.speaker;
      const time = msg.timestamp;
      const text = msg.text || (msg.dictionaryData ? `Dictionary: ${msg.dictionaryData.word}` : '');
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `rtvt-transcript-${new Date().toISOString().split('T')[0]}.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  const speakText = (text: string, langCode: string, onEndCallback?: () => void) => {
    ...
    utterance.onend = () => { setIsSpeaking(false); utteranceRef.current = null; if (onEndCallback) onEndCallback(); };
    utterance.onerror = (e) => { 
        if (e.error !== 'interrupted' && e.error !== 'canceled') console.warn("TTS Error:", e);
        setIsSpeaking(false); 
    ...
    if (isListening) {
      ...
      if (speechTimeoutRef.current) {
          ...
          if (voiceBufferRef.current.trim().length > 0) {
              ...
              handleSend(voiceBufferRef.current, true, !!isTarget); 
      ...
      try {
        ...
        setTimeout(() => { try { recognitionRef.current.start(); setIsListening(true); } catch(e) {} }, 100);
      }
      ...
      recognitionRef.current.onresult = (event: any) => {
        ...
        const fullText = finalTranscript + (finalTranscript && interimTranscript ? ' ' : '') + interimTranscript;
        ...
        if (event.results[event.results.length - 1].isFinal) {
             ...
             speechTimeoutRef.current = setTimeout(() => {
                 ...
                 handleSend(finalTranscript, true, isTarget);
  ...
  const handleSend = async (textOverride: string | null = null, isVoiceInput = false, isReverseTranslate = false) => {
    ...
    const userMsg: Message = {
      ...
      image: hasImage && selectedImage ? URL.createObjectURL(selectedImage) : null,
      mode: currentMode
    };
  ...
  const switchLanguages = () => { setSourceLang(targetLang); setTargetLang(sourceLang); };
    ...
    <div className="flex flex-col h-screen bg-slate-950 text-slate-100 font-sans selection:bg-indigo-500/30 overflow-hidden">
      ...
      <SettingsDrawer isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)}>
        <div className="space-y-6">
            <div className="space-y-3">
                <h3 className="text-xs font-semibold text-slate-500 uppercase flex items-center gap-2"><Cpu className="w-3 h-3" /> Config</h3>
                <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700 space-y-3">
                    <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => setGrammarCorrection(!grammarCorrection)} className={`w-9 h-5 rounded-full relative transition-colors ${grammarCorrection ? 'bg-indigo-500' : 'bg-slate-600'}`}>
                            <div className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform ${grammarCorrection ? 'translate-x-4' : ''}`} />
                        </button>
                    </div>
                    <div className="flex items-center justify-between">
                        <span className="text-sm text-slate-300">Auto-Playback</span>
                        <button onClick={() => setAutoSpeak(!autoSpeak)} className={`w-9 h-5 rounded-full relative transition-colors ${autoSpeak ? 'bg-indigo-500' : 'bg-slate-600'}`}>
                            <div className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform ${autoSpeak ? 'translate-x-4' : ''}`} />
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-3">
                <h3 className="text-xs font-semibold text-slate-500 uppercase flex items-center gap-2"><Gauge className="w-3 h-3" /> Audio Settings</h3>
                <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700 space-y-3">
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                            <span className="text-sm text-slate-300">TTS Speed: {ttsSpeed}x</span>
                        </div>
                        <input 
                            type="range" 
                            min="0.5" 
            ...
            <div className="pt-4 border-t border-slate-800 space-y-2">
                 <button onClick={clearHistory} className="w-full flex items-center justify-center gap-2 py-2 bg-red-900/20 hover:bg-red-900/40 text-red-300 text-sm rounded-lg transition-colors border border-red-900/30">
                    <Trash2 className="w-4 h-4" /> Clear History
                 </button>
                <button onClick={exportTranscript} className="w-full flex items-center justify-center gap-2 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm rounded-lg transition-colors border border-slate-700">
      ...
      <header className="flex items-center justify-between px-4 sm:px-6 py-3 bg-slate-900/95 backdrop-blur border-b border-slate-800 shrink-0 z-30 sticky top-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-base sm:text-lg tracking-tight leading-none bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400">RTVT <span className="text-[10px] sm:text-xs bg-indigo-500/20 text-indigo-300 px-1.5 rounded ml-1 align-top border border-indigo-500/20">ULTRA</span></h1>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <div className="hidden md:flex items-center gap-4 px-4 py-1.5 bg-slate-800/50 rounded-full border border-slate-700/50 shadow-inner">
             <div className="flex items-center gap-2 text-[10px] font-mono text-emerald-400"><div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse shadow-[0_0_10px_rgba(52,211,153,0.5)]"></div>ONLINE</div>
          </div>
          <button onClick={() => setIsSettingsOpen(true)} className="p-2 hover:bg-slate-800 rounded-full transition-colors relative"><Settings className="w-5 h-5 text-slate-400 hover:text-white transition-colors" /></button>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <aside className="hidden md:flex w-20 lg:w-64 flex-shrink-0 border-r border-slate-800 bg-slate-900/50 flex-col backdrop-blur-sm">
          <nav className="flex-1 p-3 space-y-2">
            {[{ id: 'VOICE', icon: Mic, label: 'Translate' }, { id: 'DICTIONARY', icon: Library, label: 'Dictionary' }, { id: 'VISION', icon: Eye, label: 'Image' }, { id: 'CONVERSATION', icon: SplitSquareVertical, label: 'Split View' }].map((mode) => (
              <button key={mode.id} onClick={() => { setActiveMode(mode.id); setViewState('CHAT'); }} className={`w-full flex items-center justify-center lg:justify-start gap-3 p-3 rounded-xl transition-all duration-200 group ${activeMode === mode.id && viewState === 'CHAT' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/30 ring-1 ring-indigo-400/30' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`} title={mode.label}>
                <mode.icon className={`w-5 h-5 shrink-0 transition-transform ${activeMode === mode.id ? 'scale-110' : 'group-hover:scale-110'}`} />
                <span className="hidden lg:block font-medium text-sm truncate">{mode.label}</span>
              </button>
            ))}
            <div className="pt-4 border-t border-slate-800 mt-2 space-y-2">
                <button onClick={() => setViewState('SAVED')} className={`w-full flex items-center justify-center lg:justify-start gap-3 p-3 rounded-xl transition-all ${viewState === 'SAVED' ? 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20' : 'text-slate-400 hover:bg-slate-800'}`}>
                    <Bookmark className="w-5 h-5 shrink-0" />
                    <span className="hidden lg:block font-medium text-sm truncate">Saved Phrases</span>
                </button>

                <div className="pt-2 border-t border-slate-800 space-y-2">
                    <button onClick={clearHistory} className="w-full flex items-center justify-center lg:justify-start gap-3 p-3 rounded-xl bg-red-900/20 hover:bg-red-900/40 text-red-300 transition-colors border border-red-900/30">
                        <Trash2 className="w-5 h-5 shrink-0" />
                        <span className="hidden lg:block font-medium text-sm truncate">Clear History</span>
                    </button>
                    <button onClick={exportTranscript} className="w-full flex items-center justify-center lg:justify-start gap-3 p-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors border border-slate-700">
                        <Download className="w-5 h-5 shrink-0" />
                        <span className="hidden lg:block font-medium text-sm truncate">Export Transcript</span>
                    </button>

                    <button onClick={handleInstallApp} className="w-full flex items-center justify-center lg:justify-start gap-3 p-3 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white transition-all shadow-lg shadow-indigo-500/20 font-medium border border-indigo-500/50">
                        <Smartphone className="w-5 h-5 shrink-0" />
                        <span className="hidden lg:block font-medium text-sm truncate">Install RTVT App</span>
                    </button>
                </div>
            </div>
          </nav>
        ...
        <section className="flex-1 flex flex-col relative w-full h-full">
                ...
                {activeMode === 'VISION' && (
                   <div className="flex-1 flex flex-col h-full bg-slate-950">
                        ...
                        <div className="w-full bg-slate-900/50 border-b border-slate-800/50 z-10 p-4">
                            <VisionUploader onImageSelect={setSelectedImage} selectedImage={selectedImage} clearImage={() => setSelectedImage(null)} />
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-6 sm:space-y-8 scroll-smooth pb-32 sm:pb-40">
                            ...
                            {visibleMessages.map((msg) => <MessageBubble key={msg.id} msg={msg} isSaved={savedMessages.some(m => m.id === msg.id)} savedMessages={savedMessages} setSavedMessages={setSavedMessages} handleToggleSave={handleToggleSave} handleDelete={handleDelete} handleGrammarAnalysis={handleGrammarAnalysis} analyzingId={analyzingId} handleCopy={handleCopy} copiedId={copiedId} handleDownloadAudio={handleDownloadAudio} downloadingId={downloadingId} speakText={speakText} setAddingTranslationId={setAddingTranslationId} />)}
                            ...
                            {isProcessing && (
                                <div className="flex gap-4 max-w-3xl mx-auto w-full animate-in fade-in slide-in-from-bottom-2 duration-300">
                                    ...
                                    <div className="flex items-center gap-3 text-xs sm:text-sm text-slate-400 bg-slate-900/50 px-4 py-3 rounded-2xl rounded-tl-none border border-slate-800">
                                        ...
                                        <span className="animate-pulse">Analyzing Image & Context...</span>
                                    </div>
                                </div>
                            )}
                            <div ref={chatEndRef} />
                    ...
                    <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-6 sm:space-y-8 scroll-smooth pb-32 sm:pb-40">
                        <div className="text-center py-6">
                            <span className="text-[10px] sm:text-xs font-mono text-slate-500 bg-slate-900/80 border border-slate-800 px-4 py-1.5 rounded-full shadow-sm">
                                {new Date().toLocaleDateString()} • {activeMode === 'DICTIONARY' ? 'Dictionary Lookup' : 'Translation Session'}
                            </span>
                        </div>
                        ...
                        <div className="flex gap-4 max-w-3xl mx-auto w-full animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="flex items-center gap-3 text-xs sm:text-sm text-slate-400 bg-slate-900/50 px-4 py-3 rounded-2xl rounded-tl-none border border-slate-800">
                                ...
                                <span className="animate-pulse">
                                    {activeMode === 'DICTIONARY' ? 'Searching Dictionary...' : grammarCorrection ? 'Improving & Translating...' : 'Translating...'}
                                </span>
                            </div>
                        </div>
                        )}
          ...
          {viewState === 'SAVED' && (
             <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-24">
                <h2 className="text-xl font-bold text-yellow-400 flex items-center gap-2 mb-6 px-2"><Bookmark className="w-6 h-6" /> Saved Translations</h2>
                {savedMessages.length === 0 ? (
                    ...
                    savedMessages.map(msg => <MessageBubble key={msg.id} msg={msg} isSaved={savedMessages.some(m => m.id === msg.id)} savedMessages={savedMessages} setSavedMessages={setSavedMessages} handleToggleSave={handleToggleSave} handleDelete={handleDelete} handleGrammarAnalysis={handleGrammarAnalysis} analyzingId={analyzingId} handleCopy={handleCopy} copiedId={copiedId} handleDownloadAudio={handleDownloadAudio} downloadingId={downloadingId} speakText={speakText} setAddingTranslationId={setAddingTranslationId} />)
                )}
             </div>
          )}
          ...
          {viewState === 'CHAT' && (
            ...
            <div className="max-w-3xl mx-auto relative">
              ...
              {showLanguageControls && (
                  <div className="absolute bottom-full left-0 right-0 mb-3 z-30 animate-in fade-in slide-in-from-bottom-2 zoom-in-95 duration-200">
                      <div className="flex items-center justify-between bg-slate-900/95 backdrop-blur-xl p-1.5 rounded-2xl border border-slate-700/50 max-w-sm mx-auto w-full shadow-2xl ring-1 ring-black/50">
                        <button onClick={() => setActiveSelector('source')} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg hover:bg-slate-800 transition-colors min-w-0 group">
                            <span className="text-xl grayscale group-hover:grayscale-0 transition-all">{sourceLang.flag}</span>
                            <span className="font-medium text-slate-300 truncate text-xs sm:text-sm">{sourceLang.name.split(' ')[0]}</span>
                        </button>
                        <button onClick={switchLanguages} className="p-2 rounded-full hover:bg-indigo-500/20 text-slate-500 hover:text-indigo-400 transition-all hover:rotate-180 shrink-0">
                            <ArrowRightLeft className="w-4 h-4" />
                        </button>
                        <button onClick={() => setActiveSelector('target')} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg hover:bg-slate-800 transition-colors min-w-0 group">
                            <span className="text-xl grayscale group-hover:grayscale-0 transition-all">{targetLang.flag}</span>
                            <span className="font-medium text-slate-300 truncate text-xs sm:text-sm">{targetLang.name.split(' ')[0]}</span>
                        </button>
                      </div>
                  </div>
              )}
              ...
              <div className="relative group">
                <div className={`flex items-center gap-2 bg-slate-900/90 backdrop-blur-md p-2 rounded-[20px] border transition-all duration-300 ${isListening ? 'border-red-500/50 shadow-[0_0_30px_rgba(239,68,68,0.15)] ring-1 ring-red-500/20' : 'border-slate-700 focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500/50 shadow-xl'}`}>
                  
                  {/* Left Icon (Visualizer or Language Toggle) */}
                  <div className="pl-1 sm:pl-2 shrink-0">
                      {isListening ? (
                          <div className="w-8 h-8 sm:w-10 flex items-center justify-center"><AudioVisualizer isActive={true} /></div>
                      ) : (
                          <button 
                            onClick={() => setShowLanguageControls(!showLanguageControls)}
                            className={`p-2 sm:px-3 sm:py-2 transition-all border border-white/20 duration-200 rounded-full flex items-center gap-2 ${showLanguageControls ? 'bg-indigo-500/20 text-indigo-300 ring-1 ring-indigo-500/30' : 'text-slate-400 hover:text-indigo-400 bg-indigo-500/10'}`}
                          >
                              ...
                              {!showLanguageControls && (
                                <div className="hidden sm:flex items-center gap-1.5 text-[10px] font-medium border-l border-slate-700 pl-2 ml-1 opacity-70">
                                    <span className="grayscale">{sourceLang.flag}</span>
                                    <span className="text-slate-500">→</span>
                                    <span className="grayscale">{targetLang.flag}</span>
                                </div>
                              )}
                          </button>
                      )}
                  ...
                  <input 
                      ...
                      placeholder={
                          activeMode === 'VISION' ? "Describe this image..." : 
                          activeMode === 'DICTIONARY' ? "Enter a word or sentence..." : 
                  ...
                  <div className="flex items-center gap-1.5 pr-1 shrink-0">
                     {activeMode !== 'DICTIONARY' && (
                         <button 
                            ...
                            className={`p-2.5 rounded-xl transition-all ${grammarCorrection ? 'text-indigo-300 bg-indigo-500/20 ring-1 ring-indigo-500/30' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'}`} 
                     ...
                     <button 
                        ...
                        className={`p-3 sm:p-3.5 rounded-xl transition-all duration-300 shadow-lg ${isListening ? 'bg-red-500 text-white hover:bg-red-600 scale-105 shadow-red-500/30' : 'bg-indigo-600 text-white hover:bg-indigo-500 shadow-indigo-500/30 hover:scale-105'}`}
                     > 
      ...
      {viewState !== 'CONVERSATION' && (
        <MobileNav activeMode={activeMode} setActiveMode={setActiveMode} setViewState={setViewState} />
      )}
    </div>
  );
}