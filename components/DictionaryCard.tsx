...
export const DictionaryCard = ({ data }: { data: DictionaryData }) => {
  return (
    <div className="w-full mx-auto flex float-left flex-col bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="border-b border-slate-800 pb-4 mb-4">
        ...
        {data.translation && (
          <p className="text-emerald-400 text-lg leading-relaxed font-medium">
            {data.translation}
          </p>
        )}
      </div>
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-2">
              {data.examples.map((ex, i) => (
                <li
                  key={i}
                  className="text-slate-300 text-sm pl-4 border-l-2 border-slate-700 italic"
                >
                  "{ex}"
                </li>
              ))}
        ...
        {data.synonyms && data.synonyms.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {data.synonyms.map((syn, i) => (
              <span
                key={i}
                className="px-2.5 py-1 rounded-full bg-slate-800 text-slate-300 text-xs border border-slate-700 hover:border-slate-600 transition-colors"
              >
                {syn}
              </span>
            ))}
