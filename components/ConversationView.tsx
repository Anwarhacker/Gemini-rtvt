...
import { Mic, MicOff, Plus, ChevronDown } from 'lucide-react';
...
export const ConversationView: React.FC<ConversationViewProps> = ({
    ...
    const lastGuestMsg = [...visibleMessages].reverse().find(m => m.speaker.includes('System'));
    const lastUserMsg = [...visibleMessages].reverse().find(m => m.speaker === 'User' || (m.speaker === 'System' && !m.speaker.includes('User')));

    return (
        <div className="flex flex-col h-full rounded-b-3xl md:rounded-none overflow-hidden">
            <div className="flex-1 bg-slate-900 flex flex-col items-center justify-center relative border-b border-slate-800 p-4">
                <div className="w-full h-full flex flex-col justify-center items-center">
                    {lastGuestMsg ? (
                        ...
                        <div className="text-center space-y-4 max-w-lg px-4">
                            <p className="text-lg sm:text-xl text-white">Waiting for input...</p>
                        </div>
                    )}
                </div>
            ...
            <div className="flex-1 bg-slate-950 flex flex-col items-center justify-center relative p-4">
                ...
                <div className="text-center space-y-4 max-w-lg w-full px-4">
                    {lastGuestMsg?.text ? (
                        ...
                        <p className="text-lg sm:text-xl text-white">Tap mic to speak...</p>
                    )}
